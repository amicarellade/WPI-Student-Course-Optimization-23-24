...
def format_solution(x, y, base_dict, program_keys, courses_taken_dict):
    ...
    raw_y = raw_y_sol(y)
    raw_x = raw_x_sol(x)
    print("Courses Taken Dict: " + str(courses_taken_dict))
    print("X: " + str(x))
    print("RAW X: " + str(raw_x))
...
def raw_x_sol(x):
    ...
    for key, val in x.items():
        ...
        print("key: " + str(key) + " val_num: " + str(val_num))
        if val_num > 0:
...
def calc_credit_metrics(stageI_obj, credits_taken, orig_credits_used, majors):
    num_majors = len(majors)
    mqp_credits = 0  # 9 + 3 * (num_majors - 1)
    pe_credits = 0  # 3

    min_needed = max(orig_credits_used, 45)
    anticip_total = stageI_obj + credits_taken
    ...
    #wrong lol
    # if "_MASTER" in majors[0]:
    #     num_left = stageI_obj + free_credits + mqp_credits + pe_credits
    #
    # else:
...
def apply_to_programs(sol_courses, x_dict, base_dict, program_keys, to_take):
    ...
    # apply courses
    # result_dict["ALL_MAJORS"] = apply_courses_helper("ALL_MAJORS", program_keys, sol_courses, track_applied, x_dict,
    #                                                  base_dict, to_take)
    for each_program in program_keys:
        result_dict[each_program] = apply_courses_helper(each_program, [each_program], sol_courses, track_applied,
                                                         x_dict, base_dict, to_take)
...
def apply_courses_helper(program_key, programs_to_count_toward, sol_courses, track_applied, x_dict, base_dict, to_take):
    ...
    for req, req_attrs in current_reqs_dict.items():
        ...
        for bucket_key, bucket_num in req_assigns.items():
            # print(req_assigns)
            print(bucket_key, bucket_num)
            print(sol_courses)
            bucket_courses = sol_courses[bucket_key]
            print(bucket_courses)
