...
#add the 2023 logic (flip order, etc.)
...
for i in range(len(dfs)):
    ...
    for index, row in data.iterrows():
        ...
        if major not in reqs['major'].values or req not in reqs['req_name'].values:
            # print(reqs["major"])
            # print(major)
            reqs.loc[len(reqs.index)] = [major, req, course]
        ...
        elif course[0] not in list(reqs[reqs['req_name'] == req]['req_courses'])[0]:
...
#print(reqs['req_name'])
print(reqs)
reqs.to_csv('studentdata/reqs_all_lists.csv')
