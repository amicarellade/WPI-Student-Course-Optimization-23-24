...
def run_model(program_keys, courses_taken_dict, base_dict, name_prefix, write_LP=True):
    if "_MASTER" in program_keys[0]:
        import create_model_masters as cm

    elif "_BSMS" in program_keys[0]:
        import create_model_bsms as cm
    else:
        import create_model as cm
    model = LpProblem("SchedulingCourses", LpMinimize)
    vars_dict = cm.setup_x_y_vars(model, base_dict, program_keys, courses_taken_dict)
    cm.add_requirement_constraints(model, vars_dict["X"], vars_dict["Z"], program_keys, base_dict, courses_taken_dict)
    track_SRs = cm.add_sreqs(model, vars_dict["X"], program_keys, base_dict)

    # passing X dict for BSMS
    cm.set_objective(model, vars_dict["X"], vars_dict["Y"], vars_dict["Z"], base_dict["Buckets"], 1, program_keys, courses_taken_dict)
    ...
    # passing X dict for BSMS
    cm.set_objective(model, vars_dict["X"], vars_dict["Y"], vars_dict["Z"], base_dict["Buckets"], 2, program_keys, courses_taken_dict, max_credits = min_cred)
