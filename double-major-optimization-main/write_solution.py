...
def write_output_file(file_name, solution, program_keys, run_times, counts_dict, major_names):
    ...
    with open(save_name, "w") as sol_f:
        ...
        for each_program in effective_keys:
            ...
            sol_f.write("\n \n")
            if program_name == "General Education and Projects":
                sol_f.write("MQP")
                sol_f.write("\n")
                sol_f.write("----------------------------\n")
                print("my number: "+str(int(counts_dict["MQP"]/3)))
                for i in range(int(counts_dict["MQP"]/3)):
                    sol_f.write("    ")
                    sol_f.write("[MQP]")
                    sol_f.write("\n")
                sol_f.write("\n")
                sol_f.write("Physical Education")
                sol_f.write("\n")
                sol_f.write("----------------------------\n")
                for i in range(4):
                    sol_f.write("    ")
                    ...
                    sol_f.write("\n")
                sol_f.write("\n")
            print("ITEMS: "+str(solution[each_program].items()))
